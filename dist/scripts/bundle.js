...
const out = (0, node_path_1.resolve)(root, 'deploy');
 ...
 */
function productionPackageJson() {
    const source = JSON.parse((0, node_fs_1.readFileSync)((0, node_path_1.resolve)(root, 'package.json'), 'utf8'));
    const dependencies = {};
    for (const name of Object.keys(source.dependencies ?? {})) {
        const installed = JSON.parse((0, node_fs_1.readFileSync)((0, node_path_1.resolve)(root, 'node_modules', name, 'package.json'), 'utf8'));
...
function main() {
    ...
    (0, node_child_process_1.execFileSync)(process.execPath, [require.resolve('typescript/bin/tsc'), '-p', 'tsconfig.json'], {
        stdio: 'inherit',
    });
    (0, node_fs_1.rmSync)(out, { recursive: true, force: true });
    (0, node_fs_1.mkdirSync)(out, { recursive: true });
    for (const entry of INCLUDE) {
        const source = (0, node_path_1.resolve)(root, entry);
        if (!(0, node_fs_1.existsSync)(source)) {
            console.warn(`  skipped (missing): ${entry}`);
            continue;
        }
        (0, node_fs_1.cpSync)(source, (0, node_path_1.resolve)(out, entry), { recursive: true });
        console.log(`  added: ${entry}`);
    }
    (0, node_fs_1.writeFileSync)((0, node_path_1.resolve)(out, 'package.json'), productionPackageJson(), 'utf8');
    console.log('  added: package.json (runtime dependencies only)');
    (0, node_fs_1.writeFileSync)((0, node_path_1.resolve)(out, 'index.js'), ENTRY_SHIM, 'utf8');
    console.log('  added: index.js (entry shim)');
    if (offline)
        installDependencies();
    (0, node_fs_1.writeFileSync)((0, node_path_1.resolve)(out, 'DEPLOY.md'), instructions(), 'utf8');
    console.log('  added: DEPLOY.md');
    zip();
    console.log('\nBundle ready:');
    console.log(`  ${out}`);
    console.log(`  ${(0, node_path_1.resolve)(root, 'deploy.zip')}`);
...
function installDependencies() {
    ...
    (0, node_child_process_1.execFileSync)(process.execPath, [cli, ...args], { cwd: out, stdio: 'inherit' });
    (0, node_fs_1.rmSync)((0, node_path_1.resolve)(out, 'package.json'), { force: true });
    (0, node_fs_1.rmSync)((0, node_path_1.resolve)(out, 'package-lock.json'), { force: true });
...
function npmCliPath() {
    const nodeDir = (0, node_path_1.dirname)(process.execPath);
    const candidates = [
        (0, node_path_1.resolve)(root, 'node_modules', 'npm', 'bin', 'npm-cli.js'),
        (0, node_path_1.resolve)(nodeDir, 'node_modules', 'npm', 'bin', 'npm-cli.js'),
        (0, node_path_1.resolve)(nodeDir, '..', 'lib', 'node_modules', 'npm', 'bin', 'npm-cli.js'),
    ];
...
function zip() {
    try {
        (0, node_child_process_1.execFileSync)('tar', ['-a', '-c', '-f', (0, node_path_1.resolve)(root, 'deploy.zip'), '-C', out, '.'], {
            stdio: 'ignore',
        });
