import type { CategoryDefinition, ChannelDefinition, RoleKey, ServerBlueprint } from '../types';
import { selfRoleDefinitions } from './selfRoles';
import {
  ADMIN_GUILD_PERMISSIONS,
  CURATE,
  DISCUSS,
  DISCUSS_UNVERIFIED,
  HIDDEN_FROM_EVERYONE,
  MEMBER_GUILD_PERMISSIONS,
  MODERATOR_GUILD_PERMISSIONS,
  POST,
  REACT,
  READ,
  STAGE_AUDIENCE,
...
const STAFF: RoleKey[] = ['moderator', 'admin', 'founder'];
/** Anyone who can run the server. */
const LEADERSHIP: RoleKey[] = ['admin', 'founder'];
/** Anyone who curates educational material or hosts sessions. */
const EDUCATORS: RoleKey[] = ['mentor', ...STAFF];
/** Every verified human. `newMember` is deliberately excluded. */
const COMMUNITY: RoleKey[] = ['member', 'researcher', 'mentor', ...STAFF];
 ...
 * away in a chat.
 * ──────────────────────────────────────────────────────────── */
export const SERVER: ServerBlueprint = {
  joinRole: 'newMember',
  verifiedRole: 'member',
  ...
  roles: [
    ...
    },
    {
      ...
      permissions: ADMIN_GUILD_PERMISSIONS,
    ...
    },
    {
      ...
      hoist: true,
      mentionable: false,
      permissions: MODERATOR_GUILD_PERMISSIONS,
    ...
    },
    {
      ...
      permissions: merge(MEMBER_GUILD_PERMISSIONS, ['ManageEvents']),
    ...
    },
    {
      ...
      hoist: false,
      mentionable: true,
      permissions: MEMBER_GUILD_PERMISSIONS,
  ...
  categories: [
    /* ── 1. START HERE ─────────────────────────────────────── */
    {
      ...
      overwrites: [
        ...
        ...allowRoles(LEADERSHIP, POST),
        { target: self, allow: merge(READ, POST) },
      ],
      channels: [
        {
          key: 'welcome',
        ...
        },
        {
          ...
          type: 'text',
          topic:
            'Pick your language and the order flow software you actually run. Funded verification is requested here too.',
          overwrites: [
            { target: everyone, deny: ['ViewChannel'] },
            ...allowRoles(COMMUNITY, READ, REACT, ['UseApplicationCommands']),
          ],
        },
      ],
    },
    ...
    /* ── 2. THE DESK ───────────────────────────────────────── */
    {
      ...
      overwrites: [
        HIDDEN_FROM_EVERYONE,
        ...allowRoles(COMMUNITY, DISCUSS),
        ...allowRoles(STAFF, CURATE),
      ],
      channels: [
        {
          key: 'general',
          ...
          overwrites: [{ target: role('newMember'), allow: DISCUSS_UNVERIFIED }],
        },
        {
          key: 'trading-floor',
          name: '📈│trading-floor',
          type: 'text',
          topic:
            'Live market discussion: ideas, setups, structure, order flow, execution, risk, psychology. Ideas are argued, not asserted.',
        },
        {
          ...
          type: 'forum',
          topic:
            'One post per trade or per study. Trades: context, thesis, entry, risk, management, exit, result. Studies: hypothesis, data, period, method — then the result. Tag it so it can be found again. "Rejected" is a successful outcome.',
          tags: [
            'Trade review',
            'Hypothesis',
            'Backtest',
          ...
          overwrites: [
            { target: role('researcher'), allow: ['ManageThreads', 'CreatePrivateThreads'] },
          ],
        },
        {
          ...
          type: 'text',
          topic:
            'Automated daily market digest: central banks, macro releases, statistics. Discuss inside the thread on each digest.',
          overwrites: [
            ...denyRoles(COMMUNITY, ['SendMessages', 'CreatePublicThreads']),
            ...allowRoles(COMMUNITY, THREAD_REPLY),
            ...allowRoles(EDUCATORS, POST, THREAD_CREATE),
            { target: self, allow: merge(READ, POST, THREAD_CREATE) },
          ],
        },
      ],
    },
    ...
    /* ── 3. EDUCATION ──────────────────────────────────────── */
    {
      ...
      overwrites: [
        ...
        ...allowRoles(COMMUNITY, READ, REACT, THREAD_REPLY, ['UseApplicationCommands']),
        ...
        ...denyRoles(COMMUNITY, ['SendMessages', 'CreatePublicThreads']),
        ...allowRoles(EDUCATORS, POST, THREAD_CREATE, CURATE),
      ],
      channels: [
        {
          key: 'education',
          ...
          overwrites: [...allowRoles(COMMUNITY, POST, THREAD_CREATE)],
        },
        {
          ...
          type: 'forum',
          topic:
            'Lessons, books, papers, tools, datasets and video. One post per item, tagged. Say why it is worth the time — a link with no argument is noise.',
          tags: [
            'Lesson',
            'Book',
            'Paper',
    ...
    /* ── 4. LIVE DESK ──────────────────────────────────────── */
    {
      ...
      channels: [
        ...
        },
        {
          ...
          overwrites: [...allowRoles(COMMUNITY, VOICE), ...allowRoles(EDUCATORS, VOICE_HOST)],
        },
        {
          ...
          overwrites: [
            { target: everyone, deny: ['ViewChannel', 'Connect'] },
            ...denyRoles(COMMUNITY, ['Connect', 'ViewChannel']),
            ...allowRoles(STAFF, VOICE, VOICE_HOST, ['ViewChannel', 'Connect']),
          ],
        },
        {
          ...
          type: 'stage',
          overwrites: [
            ...allowRoles(COMMUNITY, STAGE_AUDIENCE),
            ...
            ...denyRoles(COMMUNITY, ['Speak', 'Stream']),
            ...allowRoles(EDUCATORS, STAGE_HOST),
          ],
        },
      ],
    },
      ...
      channels: [
        {
          ...
          type: 'announcement',
          topic:
            'Market Reviews, Weekly Reviews, Replay Sessions, Strategy Lab, AMAs, guest sessions. Managed with /event.',
        },
      ],
    },
    ...
    /* ── 6. ITALIA ─────────────────────────────────────────── */
    {
      ...
      channels: [
        ...
        },
        {
          ...
          type: 'text',
          topic:
            'Discussione di mercato in italiano: idee, struttura, esecuzione, rischio. Le review complete vanno in {{#the-lab}}, in inglese.',
        },
        {
          ...
          overwrites: [...allowRoles(COMMUNITY, VOICE)],
        },
      ],
    },
    ...
    /* ── 7. SUPPORT ────────────────────────────────────────── */
    {
      ...
      overwrites: [
        ...
        ...allowRoles(['newMember'], READ, THREAD_REPLY, ['UseApplicationCommands']),
        ...allowRoles(STAFF, POST, CURATE, ['CreatePrivateThreads', 'ManageThreads']),
        { target: self, allow: merge(READ, POST, ['CreatePrivateThreads', 'ManageThreads']) },
      ],
      channels: [
        {
          key: 'faq',
        ...
        },
        {
          ...
          type: 'text',
          topic:
            'Press the button to open a private ticket with the staff. Only you and the staff can read it.',
        },
      ],
    },
    ...
    /* ── 8. STAFF ──────────────────────────────────────────── */
    {
      ...
      overwrites: [
        HIDDEN_FROM_EVERYONE,
        ...allowRoles(STAFF, DISCUSS, CURATE, ['CreatePrivateThreads']),
...
export const ROLE_HIERARCHY: RoleKey[] = SERVER.roles.map((r) => r.key as RoleKey);

/** Core roles plus the self-assignable ones. Used for resolution only. */
export function allRoleDefinitions() {
  return [...SERVER.roles, ...SERVER.selfRoles];
}
...
export function allChannels(): FlatChannel[] {
  return SERVER.categories.flatMap((category) =>
    category.channels.map((channel) => ({ category, channel })),
  );
}

/** Look up a channel definition by key, or undefined if it is not managed. */
export function channelDef(key: string): FlatChannel | undefined {
