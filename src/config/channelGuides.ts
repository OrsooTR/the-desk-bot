...
export interface ChannelGuide {
  /** One line stating the channel's job. */
  headline: string;
  /** What belongs here. */
  belongs: string[];
  /** What does not. Omit where it would be obvious or preachy. */
  avoid?: string[];
  /** Optional closing line — a standard, a format, a reminder. */
  standard?: string;
}
...
export const CHANNEL_GUIDES: Record<string, ChannelGuide> = {
  roles: {
    ...
    standard:
      'Select to add a role, deselect to remove it. Funded status is checked by a human — the menus above are not.',
  },
  ...
  'trading-floor': {
    ...
    standard:
      'State what would invalidate your idea. An idea with no invalidation is not an idea, it is a hope. Finished trades and full studies go in {{#the-lab}}.',
  },
  ...
  'the-lab': {
    ...
    standard:
      'Run `/review` to open a structured trade post. Method before result: if you cannot state the sample size, you do not have a finding yet. A profitable trade taken badly is still a bad trade.',
  },
  ...
  'news-feed': {
    ...
    standard:
      'Posted every morning at 06:30 UTC from central bank and statistics feeds. Reply in the thread under a digest, or take a broader discussion to {{#trading-floor}}.',
  },
  ...
  library: {
    ...
    standard:
      'Tag your post — the tags are the syllabus, and filtering by one lets you follow a subject end to end. Run `/resources` for the curated starting list.',
  },
  ...
  'live-trading': {
    ...
    standard:
      'Open a room from **➕ Create Trading Room** — it exists only while someone is in it. Conclusions afterwards go to {{#trading-floor}} or {{#the-lab}}.',
  },
  ...
  events: {
    ...
    standard:
      'RSVP through the event above the channel list and Discord will remind you. Run `/event list` for what is coming.',
  },
  ...
  italia: {
    ...
    standard:
      'Il resto del server resta in inglese, materiale didattico compreso. Le review complete vanno in {{#the-lab}}.',
  },
  ...
  'trading-italia': {
    ...
    standard:
      'Le stesse regole del trading floor: si argomenta, non si afferma. Le review e le ricerche complete vanno in {{#the-lab}}, in inglese, così le legge tutto il server.',
  },
  ...
  tickets: {
    ...
    standard:
      'Press the button below. It opens a private thread only you and the staff can read. Do not DM moderators directly.',
  },
...
export function guideFor(channelKey: string): ChannelGuide | undefined {
  return CHANNEL_GUIDES[channelKey];
}
