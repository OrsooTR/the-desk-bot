...
import type { ResourceOutcome, SetupReport, SyncOptions } from './types';
...
export async function syncServer(guild: Guild, options: SyncOptions): Promise<SetupReport> {
...
function preflight(permissions: PermissionsBitField, report: SetupReport): void {
  const required = [
  ...
  for (const [label, flag] of required) {
    if (!permissions.has(flag)) {
      report.warnings.push(
        `I am missing the **${label}** permission. Anything requiring it will be reported as failed.`,
      );
    }
  }
}
...
const COMMUNITY_ONLY_TYPES: ManagedChannelType[] = ['announcement', 'stage', 'media'];
 ...
 */
function checkCommunityFeatures(guild: Guild, report: SetupReport): void {
  if (guild.features.includes('COMMUNITY')) return;

  const gated = allChannels().filter(({ channel }) =>
    COMMUNITY_ONLY_TYPES.includes(channel.type),
  );
  if (gated.length === 0) return;

  report.warnings.push(
    `This server is not enabled as a Community, so Discord will reject ${gated
...
async function syncEveryoneRole(
  guild: Guild,
  report: SetupReport,
  options: SyncOptions,
  reason: string,
  ...
  const desired = new PermissionsBitField(SERVER.everyonePermissions).bitfield;
  ...
  if (!options.dryRun) {
    try {
      ...
      logger.info('SETUP', 'Updated @everyone baseline permissions');
    } catch (error) {
...
async function syncRoles(
  ...
  for (const definition of [...SERVER.roles, ...SERVER.selfRoles]) {
    const existing = findRole(guild, definition.key);
    ...
    if (!existing) {
      ...
      if (!options.dryRun) {
        try {
          ...
          state.rememberRole(definition.key, created.id);
          logger.info('SETUP', `Created role: @${definition.name}`);
        } catch (error) {
    ...
    if (existing.managed) {
      ...
      report.warnings.push(
        `@${existing.name} is managed by an integration and was left untouched.`,
      );
    ...
    if (!options.dryRun) {
      try {
        ...
        state.rememberRole(definition.key, existing.id);
        logger.info('SETUP', `Updated role: @${definition.name} (${reasons.join(', ')})`);
      } catch (error) {
...
function diffRole(role: Role, definition: RoleDefinition): string[] {
  const reasons: string[] = [];
  if (role.name !== definition.name) reasons.push(`name (${role.name} → ${definition.name})`);
  if (role.permissions.bitfield !== new PermissionsBitField(definition.permissions).bitfield)
    reasons.push('permissions');
  if (role.hoist !== definition.hoist) reasons.push('hoist');
  if (role.mentionable !== definition.mentionable) reasons.push('mentionable');
  if (
    role.colors.primaryColor !== (definition.color ?? 0) ||
 ...
 */
async function syncRolePositions(
  guild: Guild,
  report: SetupReport,
  options: SyncOptions,
): Promise<void> {
  const managed = SERVER.roles
    .map((definition) => findRole(guild, definition.key))
  ...
  const assignments = managed.map((role, index) => ({ role, position: slots[index] as number }));
  ...
  if (options.dryRun) {
    report.notes.push(
      `Role order would be corrected for: ${drifted.map(({ role }) => `@${role.name}`).join(', ')}.`,
    );
    return;
  }
  ...
  try {
    ...
    logger.info('SETUP', 'Reordered roles to match the blueprint hierarchy');
  } catch (error) {
    report.warnings.push(
      'Could not reorder roles — my own role is probably below them. Drag the bot role higher and re-run.',
    );
    logger.warn('SETUP', 'Role reorder failed', { detail: describeError(error) });
  }
}
 ...
 */
function checkAssignableRoles(guild: Guild, report: SetupReport): void {
  const me = guild.members.me;
  if (!me) return;

  const mine = me.roles.highest;
  const blocked = [SERVER.joinRole, SERVER.verifiedRole]
    .map((key) => findRole(guild, key))
    .filter((role): role is Role => role !== null && role.position >= mine.position);

  if (blocked.length === 0) return;

  report.warnings.push(
    `My highest role (@${mine.name}) is not above ${blocked
      .map((role) => `@${role.name}`)
...
async function syncCategoriesAndChannels(
  guild: Guild,
  botId: string,
  report: SetupReport,
  options: SyncOptions,
  ...
  for (const [index, definition] of SERVER.categories.entries()) {
...
async function syncCategory(
  ...
  const existing = findCategory(guild, definition.key);
  const { overwrites, missing } = resolveOverwrites(guild, definition.overwrites, botId);
  ...
  if (!existing) {
    ...
    try {
      ...
      state.rememberCategory(definition.key, created.id);
      logger.info('SETUP', `Created category: ${definition.name}`);
      report.outcomes.push(outcome);
      return created;
    } catch (error) {
  ...
  const reasons: string[] = [];
  if (existing.name !== definition.name) reasons.push(`name (${existing.name} → ${definition.name})`);
  ...
  if (!options.dryRun) {
    try {
      await existing.edit({
        name: definition.name,
        permissionOverwrites: mergeOverwrites(existing.permissionOverwrites.cache, overwrites),
        reason,
      });
      state.rememberCategory(definition.key, existing.id);
      logger.info('SETUP', `Updated category: ${definition.name} (${reasons.join(', ')})`);
    } catch (error) {
...
async function syncChannel(
  ...
  const wantedType = DISCORD_TYPE[definition.type];
  ...
  const { overwrites, missing } = resolveLayered(
    guild,
    [category.overwrites, definition.overwrites ?? []],
    botId,
  );
  ...
  const existing = findChannel(guild, definition.key);
  ...
  if (!existing) {
    ...
    const mistyped = findChannelAnyType(guild, definition.key);
    if (mistyped) {
    ...
    try {
      ...
      state.rememberChannel(definition.key, created.id);
      logger.info('SETUP', `Created channel: ${label}`);
    } catch (error) {
  ...
  if (!options.dryRun) {
    ...
    try {
      ...
      state.rememberChannel(definition.key, existing.id);
      logger.info('SETUP', `Updated channel: ${label} (${reasons.join(', ')})`);
    } catch (error) {
...
function diffChannel(
  ...
  if (channel.name !== definition.name) reasons.push(`name (${channel.name} → ${definition.name})`);
  if (parent && channel.parentId !== parent.id) reasons.push('category');

  if (isTextLike(channel)) {
    if ((channel.topic ?? '') !== (definition.topic ?? '')) reasons.push('topic');
  ...
  if (isThreadOnly(channel)) {
    if ((channel.topic ?? '') !== (definition.topic ?? '')) reasons.push('guidelines');
    const missing = tagGap(channel, definition);
    if (missing.length > 0) reasons.push(`tags to add: ${missing.join(', ')}`);
    if (
      definition.layout &&
 ...
 */
async function convertTextType(
  guild: Guild,
  definition: ChannelDefinition,
  wantedType: ChannelType,
  label: string,
  report: SetupReport,
  options: SyncOptions,
  reason: string,
  ...
  if (!convertible.includes(wantedType)) return;

  const live = findChannelAnyType(guild, definition.key);
  if (!live || live.type === wantedType || !convertible.includes(live.type)) return;

  if (options.dryRun) {
    report.notes.push(
      `${label} would be converted from ${typeName(live.type)} to ${definition.type}.`,
    );
    return;
  }

  try {
    await (live as TextChannel).edit({
      type: wantedType as ChannelType.GuildText | ChannelType.GuildAnnouncement,
      reason,
    });
    report.notes.push(`${label} converted to ${definition.type}.`);
    logger.info('SETUP', `Converted ${label} to ${definition.type}`);
  } catch (error) {
    report.warnings.push(`Could not convert ${label} to ${definition.type}.`);
    logger.error('SETUP', `Type conversion failed for ${label}`, error);
  }
}

/* ── Channel type helpers ──────────────────────────────────── */

function isTextLike(channel: ManagedChannel): channel is TextChannel | NewsChannel {
  return (
    channel.type === ChannelType.GuildText || channel.type === ChannelType.GuildAnnouncement
  );
}

function isThreadOnly(channel: ManagedChannel): channel is ForumChannel | MediaChannel {
  return channel.type === ChannelType.GuildForum || channel.type === ChannelType.GuildMedia;
}

function forumLayout(layout: 'list' | 'gallery'): ForumLayoutType {
  return layout === 'gallery' ? ForumLayoutType.GalleryView : ForumLayoutType.ListView;
}

function supportsTopic(type: ManagedChannelType): boolean {
  return type === 'text' || type === 'announcement' || type === 'forum' || type === 'media';
}

function channelLabel(definition: ChannelDefinition): string {
...
function tagGap(channel: ForumChannel | MediaChannel, definition: ChannelDefinition): string[] {
 ...
 */
function tagsToWrite(
  channel: ForumChannel | MediaChannel,
  definition: ChannelDefinition,
): GuildForumTagData[] | null {
  const missing = tagGap(channel, definition);
 ...
 */
function collectUnmanaged(guild: Guild, report: SetupReport): void {
  report.unmanagedChannels = unmanagedNames(guild, managedChannelIds(guild));
}

export function managedChannelIds(guild: Guild): Set<string> {
  const ids = new Set<string>();

  for (const category of SERVER.categories) {
    const resolved = findCategory(guild, category.key);
...
function unmanagedNames(guild: Guild, managed: Set<string>): string[] {
  return [
...
function fail(outcome: ResourceOutcome, error: unknown, action: string): void {
  outcome.status = 'failed';
  outcome.error = `Could not ${action}.`;
  logger.error('SETUP', `Failed to ${action}`, error);
}
